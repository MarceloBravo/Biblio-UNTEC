...
public class UserDeleteServlet extends HttpServlet {
    ...
    ) throws ServletException, IOException 
    {
        try{
            ...
            request.setAttribute("message", result ? "Usuario eliminado con éxito" : "El usuario no pudo ser eliminado");
            request.setAttribute("code", result ? 200 : 500);
            response.sendRedirect(request.getContextPath() + "/users");
        }catch(Exception e){
            System.out.println(e);
            request.setAttribute("message", "Ocurrió un error al eliminar el usuario");
